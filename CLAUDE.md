...
Idioms the exercises expect: `docs/PATTERNS.md` · what each one teaches: `docs/LEARNING_PATH.md` · environment and troubleshooting: `docs/SETUP.md` — read these on demand, not up front; each is long and only a slice is relevant to any given task.
...
- **Every exercise has a lesson** at `docs/lessons/<slug>.md` (plus `docs/lessons/index.md`), a beginner-paced, problem-first primer that stops at a stated spoiler line — no `data-testid`, no hidden edge case, no domain object from that exercise's own README. `docs/LEARNING_PATH.md`'s bullets are the opposite: post-hoc, written to be read *after* an attempt, and free to reference the exercise's own requirements. `docs/lessons/` is inside CI's four-directory affected-aware fence (`packages/`, `solutions/`, `shared/`, `docs/`), same as every other doc.
...
- `docs/PATTERNS.md` documents idioms for every batch, 01–30. `docs/ANTI_PATTERNS.md` now covers all six batches — eighteen entries (AP-1…AP-18), each with a "Find it" grep so its "Applies to" list can be re-derived rather than trusted; re-run the greps before citing one, per the file's own note that they were last verified against `solutions/` on 2026-08-17.
- **31-panel-forwarding exists but is not yet wired up like 01–30**: it has a starter, a solution, specs (18 tests, in sync), and a lesson, but no `dev:31` script in root `package.json`, no row in root `README.md` or `docs/LEARNING_PATH.md`, and no entry in `docs/PATTERNS.md`. Treat it as in-progress — don't count it toward the "01–30" totals above until it's fully wired.
...
- **CI's `test:solutions` step is affected-aware**, not a full run every time: `.github/workflows/ci.yml` diffs against the PR's base commit (or the prior push on `main`) and runs `pnpm --filter "...[$BASE]" --filter "./solutions" --if-present test`, which is only `solutions/*` packages that changed plus dependents of anything changed (so a `shared/exercise-shell` edit still re-triggers all 30, since every solution depends on it). Any diff touching a path outside `packages/`, `solutions/`, `shared/`, `docs/` — or an unresolvable diff base — falls back to the full unfiltered `pnpm test:solutions`. Adding a new top-level directory to the repo should prompt reconsidering that four-directory fallback list; `check`, `typecheck`, `sync:tests:check`, `build`, and `docs:build` stay unfiltered on purpose.
