...
import { asWord, countExerciseTests } from './repoStats'
...
export default defineConfig({
  ...
  transformPageData(pageData) {
    if (pageData.relativePath === 'index.md') {
      const { exercises, tests } = countExerciseTests()
      pageData.frontmatter.hero.text =
        `${exercises} exercises, ${tests} tests, ${asWord(lessonsSidebar.length)} batches`
    }
  },
