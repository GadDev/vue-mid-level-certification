...
Vue's `provide`/`inject` lets an ancestor make a value available to *any*
...
has a real gap: `inject<Ref<'en' | 'fr'>>('language')` is the *caller*
...
usually the app's root component. For state that belongs to the *whole app*,
