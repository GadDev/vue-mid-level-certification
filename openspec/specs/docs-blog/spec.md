...
Every file under `docs/blog/` that represents a post SHALL declare `title`, `date`, `tags`, and `summary` in its frontmatter. `date` SHALL be an ISO `YYYY-MM-DD` string matching the date encoded in the post's filename.

#### Scenario: Post missing a required field fails the build
- **WHEN** a post under `docs/blog/` omits `title`, `date`, `tags`, or `summary` from its frontmatter
- **THEN** `pnpm docs:build` fails or the content loader errors, surfacing the problem before it reaches the deployed site
