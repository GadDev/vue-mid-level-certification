...
frontmatter block itself. Posts published before this requirement took effect are exempt from this
field until they are next revised.
...
- **THEN** that value equals `ceil(bodyWordCount / 200)` (minimum 1) for that post's actual body word
  count, not an estimate made before the post was drafted
